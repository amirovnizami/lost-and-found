...
To update the database use this command from the Web project folder (replace `Lost_And_Found` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Lost_And_Found.Infrastructure/Lost_And_Found.Infrastructure.csproj -s Lost_And_Found.Web.csproj
```
